...
class PCUtil {
 public:
    ...
    static bool MinRangeFilter(const PointCloudPtr in_cloud, double range, PointCloudPtr out_cloud) {
    ...
    static bool MaxRangeFilter(const PointCloudPtr in_cloud, double range, PointCloudPtr out_cloud) {
        out_cloud->points.clear();
        for (size_t i = 0; i < in_cloud->points.size(); i++) {
            PointType p = in_cloud->points[i];
            double dist = std::sqrt(p.x * p.x + p.y * p.y + p.z * p.z);
