...
class Util {
 public:
    ...
    static Eigen::Matrix4d GetMatrix(double x, double y, double z, double roll, double pitch, double yaw) {
    ...
    static Eigen::Matrix4d GetMatrix(const Eigen::Vector3d& translation, const Eigen::Matrix3d& rotation) {
        Eigen::Matrix4d ret = Eigen::Matrix4d::Identity();
        ret.block<3, 1>(0, 3) = translation;
        ret.block<3, 3>(0, 0) = rotation;
        return ret;
    ...
    static void QV2Matrix(const Eigen::Quaterniond& quat, const Eigen::Vector3d& vect, Eigen::Matrix4d* mat) {
        *mat = Eigen::Matrix4d::Identity();
        mat->block<3, 3>(0, 0) = quat.matrix();
        mat->block<3, 1>(0, 3) = vect;
    }

    static void Matrix2QV(const Eigen::Matrix4d& mat, Eigen::Quaterniond* quat, Eigen::Vector3d* vect) {
        *quat = Eigen::Quaterniond(mat.block<3, 3>(0, 0));
        quat->normalize();
        *vect = mat.block<3, 1>(0, 3);
    }

    static void Matrix2Isometry(const Eigen::Matrix4d& mat, Eigen::Isometry3d* iso) {
        iso->setIdentity();
        iso->linear() = mat.block<3, 3>(0, 0);
        iso->translation() = mat.block<3, 1>(0, 3);
    }

    static void Isometry2Matrix(const Eigen::Isometry3d& iso, Eigen::Matrix4d* mat) {
        *mat = Eigen::Matrix4d::Identity();
        mat->block<3, 3>(0, 0) = iso.rotation();
        mat->block<3, 1>(0, 3) = iso.translation();
    }

    static void QV2Isometry(const Eigen::Quaterniond& quat, const Eigen::Vector3d& vect, Eigen::Isometry3d* iso) {
        iso->setIdentity();
        iso->linear() = quat.toRotationMatrix();
        iso->translation() = vect;
    }

    static void Isometry2QV(const Eigen::Isometry3d& iso, Eigen::Quaterniond* quat, Eigen::Vector3d* vect) {
