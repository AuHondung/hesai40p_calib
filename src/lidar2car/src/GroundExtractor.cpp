...
bool GroundExtractor::LPRFitting(const PointCloudPtr in_cloud,
    ...
    for (size_t i = 0; i < sort_cloud->points.size(); i++) {
        size_t next_idx = static_cast<size_t>(i + 0.001 * sort_cloud->points.size());
        ...
        if ((sort_cloud->points[next_idx].z - sort_cloud->points[i].z) < 0.2) {
            sort_cloud->points.erase(sort_cloud->points.begin(), sort_cloud->points.begin() + i);
            break;
        }
    }
    ...
    for (size_t i = 0; i < sort_cloud->points.size(); i++) {
        if (sort_cloud->points[i].z <= (lpr_avg_height + lpr_least_gpoints_interval_)) {
...
bool GroundExtractor::CalArea(const PointType& p1,
    ...
    side[0] = std::sqrt(std::pow(p1.x - p2.x, 2) + std::pow(p1.y - p2.y, 2) + std::pow(p1.z - p2.z, 2));
    side[1] = std::sqrt(std::pow(p1.x - p3.x, 2) + std::pow(p1.y - p3.y, 2) + std::pow(p1.z - p3.z, 2));
    side[2] = std::sqrt(std::pow(p3.x - p2.x, 2) + std::pow(p3.y - p2.y, 2) + std::pow(p3.z - p2.z, 2));
    ...
    *area = std::sqrt(semi_perimeter * (semi_perimeter - side[0]) * (semi_perimeter - side[1]) * (semi_perimeter - side[2]));
...
bool GroundExtractor::FittingPlane(PointCloudPtr in_cloud,
                                   PlaneParam* plane) {
    Eigen::Vector3d point(in_cloud->points[0].x, in_cloud->points[0].y, in_cloud->points[0].z);
...
bool GroundExtractor::FittingPlaneMesh(const PointCloudPtr in_cloud, PlaneParam *plane) {
    ...
    for (int i = 0; i < rr_iter_times_; i++) {
        for (int j = 0; j < rr_max_rand_iters_; j++) {
            ...
            for (size_t j = 0; j < range; j++) {
                ...
                // 点云其他点到该平面的距离
                double point_to_plane_dis =
                    std::fabs(pp.normal(0) * point.x + pp.normal(1) * point.y + pp.normal(2) * point.z + pp.intercept);
                if (point_to_plane_dis <= rr_fit_dist_thre_) {
                    inlier_points++;
                }
            }
        ...
        // random search to refine parameter 随机搜索->用两套不同的参数进行两次细化搜索 -> 目的增加地面内点
